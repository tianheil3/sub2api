      ...
      >
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div class="flex items-end">
            <div>
              <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <Toggle v-model="newPlan.auto_disable_on_unauth" />
                {{ t('admin.scheduledTests.autoDisableOnUnauth') }}
              </label>
              <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">
                {{ t('admin.scheduledTests.autoDisableOnUnauthHelp') }}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              ...
              <!-- Auto Disable on 401 Badge -->
              <span
                v-if="plan.auto_disable_on_unauth"
              ...
              >
                {{ t('admin.scheduledTests.autoDisableOnUnauthBadge') }}
              </span>
            </div>
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div class="flex items-end">
                <div>
                  <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                    <Toggle v-model="editForm.auto_disable_on_unauth" />
                    {{ t('admin.scheduledTests.autoDisableOnUnauth') }}
                  </label>
                  <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">
                    {{ t('admin.scheduledTests.autoDisableOnUnauthHelp') }}
...
const resetNewPlan = () => {
  ...
  newPlan.auto_disable_on_unauth = false
}
...
const handleCreate = async () => {
  ...
  try {
    ...
    await adminAPI.scheduledTests.create({
      ...
      auto_disable_on_unauth: newPlan.auto_disable_on_unauth
    })
...
const startEdit = (plan: ScheduledTestPlan) => {
  ...
  editForm.auto_disable_on_unauth = plan.auto_disable_on_unauth
}
...
const handleEdit = async () => {
  ...
  try {
    const updated = await adminAPI.scheduledTests.update(editingPlanId.value, {
      ...
      auto_disable_on_unauth: editForm.auto_disable_on_unauth
    })
