...
describe('UseKeyModal', () => {
  it('renders Grok Build and OpenCode setup for Grok groups', async () => {
    ...
    const opencodeConfig = wrapper.findAll('pre code')
      .map((code) => code.text())
      .find((content) => content.includes('"provider"'))
    expect(opencodeConfig).toBeDefined()
    const jsonStart = opencodeConfig!.indexOf('{')
    const jsonEnd = opencodeConfig!.lastIndexOf('}')
    const parsed = JSON.parse(opencodeConfig!.slice(jsonStart, jsonEnd + 1))
  ...
  it('renders a one-click setup script for Codex config', () => {
    ...
    expect(wrapper.text()).toContain('keys.useKeyModal.oneClick.title')
    expect(wrapper.text()).toContain('keys.useKeyModal.oneClick.recommended')
    expect(wrapper.text()).toContain('keys.useKeyModal.orManual')
  ...
  it('renders the Windows Codex setup script without inline here-string terminators', async () => {
    ...
    const codeBlock = wrapper.find('pre code')
    expect(codeBlock.exists()).toBe(true)
    expect(codeBlock.text()).toContain('$configToml = "model_provider = `"OpenAI`"')
    expect(codeBlock.text()).toContain('$authJson = "{`n')
    expect(codeBlock.text()).toContain('[System.IO.File]::WriteAllText((Join-Path $codexDir \'config.toml\'), $configToml, [System.Text.UTF8Encoding]::new($false))')
    expect(codeBlock.text()).toContain('[System.IO.File]::WriteAllText((Join-Path $codexDir \'auth.json\'), $authJson, [System.Text.UTF8Encoding]::new($false))')
  ...
  it('renders GPT-5.6 alias and max variants in OpenCode config', async () => {
    ...
    const config = wrapper.findAll('pre code')
      .map((code) => code.text())
      .find((content) => content.trimStart().startsWith('{') && content.includes('"gpt-5.6"'))
